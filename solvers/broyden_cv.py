...
def broyden(g: Callable, x0: jnp.ndarray, max_iter: int, eps: float, *args) -> dict:
    ...
    def body_fun(state: _BroydenResults):
        # Get an estimate for the inverse jacobian
        inv_jacobian = -matvec(state.Us, state.VTs, state.gx)
        # Do a line search using an update rule
        # - inv_jacobian is the direction of the update
        ...
        # TODO: Why is the objective to control the vector norm of the gx matrix?
